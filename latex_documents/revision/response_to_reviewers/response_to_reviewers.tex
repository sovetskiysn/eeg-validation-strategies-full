...
\begin{itemize}
	\item \textbf{Revised title.} The manuscript has been retitled from
	      ``Evaluation of validation strategies and transferability of EEG
	      attention decoding: a comparative analysis on open datasets'' to
	      \textbf{``Zero-shot generalization of EEG attention-state decoding
	      across increasingly stringent validation strategies on open
	      datasets''} (the heading above retains the submitted title for
	      identification). The new title replaces ``transferability'' with the
	      \textbf{zero-shot, pre-adaptation} framing that is actually
	      evaluated, names the target as \textbf{attention-state decoding} to
	      match the operational high-/low-attention labels used throughout the
	      revised text, and states that the protocols are compared under
	      \textbf{increasingly stringent validation}.
	\item \textbf{Reformulated Introduction and study scope.} The Introduction
	      was rewritten in full around an \textbf{explicit research goal}, the
	      \textbf{knowledge gap} it addresses and a \textbf{contribution} the
	      data can support: the paper now claims \textbf{discrimination between
	      operationally defined experimental conditions rather than decoding of
	      attention as a psychological construct}. ``Results and analysis'' 
	      follows the same scope and is organized around \textbf{two focused
	      questions} --- how performance behaves under increasingly stringent
	      validation, and whether the effects hold across decoders --- each
	      answered by a dedicated aggregate figure.
	\item \textbf{Additional models and a sensitivity analysis across them.}
	      The model set was expanded from the single gradient-boosting decoder
	      to \textbf{five classifiers spanning the linear, tree-based and
	      deep-learning families} --- \textbf{logistic regression},
	      \textbf{XGBoost}, \textbf{ShallowFBCSPNet}, \textbf{EEGNet} and
	      \textbf{EEG Conformer} --- and every scenario is now repeated with
	      each of them, making the \textbf{decoder an explicit factor of the
	      design}. An effect reproduced across all five is attributable to the
	      data and the protocol rather than to one learner, which is what the
	      \textbf{sensitivity analysis} in ``Performance consistency across
	      decoders'' reports.
	\item \textbf{Leakage-free validation throughout.} All identified sources
	      of data leakage were eliminated, and every reported protocol is now
	      free of leakage and of subject-identity confounds. The within-domain
	      baseline is \textbf{grouped by the physical recording unit} --- a
	      \textbf{session} in the mental attention-state dataset, a
	      \textbf{run-level unit} in SAM-40, since the datasets define
	      acquisition differently --- so overlapping windows from one
	      ...
	      target composition of the held-out participant. The cross-dataset
	      protocol drew no protocol-level objection and is unchanged; the
	      concerns raised about it, label semantics and the reduced montage, are
	      addressed by the corrected label mapping and the narrowed
	      interpretation.
