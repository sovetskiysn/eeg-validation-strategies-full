...
We thank the Editor for this reminder. The revised manuscript was rebuilt on
the official PLOS ONE \LaTeX{} template and now follows both style samples.

\emph{Title, authors and affiliations.} A sentence-case title of 125 characters,
author names without titles or degrees, numbered superscript affiliations below
the author line, and an asterisk marking the corresponding author with the
contact email. The abstract is 269 words.

\emph{Main body.} The manuscript is double-spaced with continuous line
numbering, uses the supplied \texttt{plos2025.bst} style with unbracketed
reference numbering, and labels figure captions as ``Fig~N.'' in bold with the
caption title in the required form. The end matter follows the prescribed
order: references, then a legend for each supporting item, which is uploaded as
its own file and cited in the text as ``S1 Table''--``S4 Table''. Funding
information has been removed from the manuscript file, as PLOS ONE requires it
only in the Financial Disclosure field.

\emph{Figures and file naming.} All three figures have been redrawn to fit the
PLOS dimension limits without removing any panel, column or row: the transfer
matrix still reports all eleven source--target columns for five decoders, and
the other two figures still compare all five decoders across every validation
protocol. They now measure 6.5$\times$6.7, 7.3$\times$3.7 and
7.5$\times$8.2~inches, within the 7.5~inch width and 8.75~inch height limits,
with all in-figure text at 8--12~pt. The manuscript file contains no embedded
graphics, and each figure is uploaded separately as \texttt{Fig1.tif},
\texttt{Fig2.tif} and \texttt{Fig3.tif} in LZW-compressed TIFF at 300~dpi.

\emph{Section structure.} The methods appear as two consecutive sections,
\emph{Open-access datasets} and \emph{Methodology}, rather than under a single
\emph{Materials and methods} heading, because the study compares two
independently collected public datasets whose protocols and cohorts need a
self-contained description before the harmonization built on them can be
followed. The prescribed order of content is otherwise preserved, and we are
happy to merge the two sections if the Editor prefers strict conformity.
...
\textbf{[TODO for supervisor --- we do not have access to the grant records;
please provide the correct grant number(s) for `Funding Information' so it
matches `Financial Disclosure' (Grant no. BR27198099).]}
...
\textbf{[TODO for supervisor --- we cannot confirm the funder's actual role;
please approve the standard statement ``The funders had no role in study
design, data collection and analysis, decision to publish, or preparation of
the manuscript.'' or supply the correct one, for use here and in the cover
letter.]}
