...
\textbf{[TODO for supervisor --- this item is handled on the submission-form
side, not in the manuscript: the `Data Availability' field in Editorial Manager
has to be filled in with the repository and identifier of every source. Everything
needed for that is collected below; please check that the links point to exactly
the versions and files we analysed, then enter them in the field and finalise
the response text.]}

\vspace{0.2cm}

We thank the Editor for this suggestion. The sources are the following:

\begin{itemize}
	\item \textbf{Dataset~A} --- \emph{EEG data for Mental Attention State
	      Detection}. Repository: Kaggle. Direct link:\\
	      \url{https://www.kaggle.com/datasets/inancigdem/eeg-data-for-mental-attention-state-detection}
	\item \textbf{Dataset~B} --- \emph{SAM~40}. Repository: Figshare. DOI:\\
	      \url{https://doi.org/10.6084/m9.figshare.14562090.v1}
	\item \textbf{Author-generated code and per-run artifacts} --- the analysis
	      code, the configuration of every reported run, and the artifacts
	      underlying the reported figures and tables, released under the MIT
	      license. Repository: GitHub. Direct link:\\
	      \url{https://github.com/sovetskiysn/eeg-validation-strategies}\\
	      Archive: Zenodo. DOI: \url{https://doi.org/10.5281/zenodo.22255472}
\end{itemize}
...
We thank the Editor for this note. Each Supporting Information item will be
uploaded as its own file under the `Supporting Information' file type rather
than `Figure', and we have confirmed that every item carries an
``S1~Table''--``S4~Table'' legend after the reference list, in the
manuscript's ``Supporting information'' section. Each is also cited by that
label at first mention in the Results (Section~``Results and analysis'').
