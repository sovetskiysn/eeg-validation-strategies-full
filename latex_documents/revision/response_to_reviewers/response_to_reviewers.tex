...
the official PLOS ONE \LaTeX{} template, and we did our best to follow both
style samples: title, author and affiliation block, abstract, spacing and line
numbering, reference style, figure captions and in-text citations, end-matter
order, Supporting Information items, figure specifications, and file naming.

One permitted deviation is worth noting. The methods appear as two consecutive
sections, \emph{Open-access datasets} and \emph{Methodology}, rather than under
a single \emph{Materials and methods} heading, because the two public datasets
need a self-contained description before the harmonization built on them can be
followed. The prescribed order of content is otherwise preserved, and we are
happy to merge the two sections if the Editor prefers.
...
\textbf{[TODO for supervisor --- please verify this statement before
submission: that the two dataset links resolve to exactly the versions and
files analysed here, and that no further author-generated data need to be
listed. The wording below is then to be pasted into the `Data Availability'
field of the online submission form.]}

\vspace{0.2cm}

We thank the Editor for this suggestion and provide the missing details now.
Both datasets analysed in this study are third-party open datasets, and the
revised Data Availability Statement now identifies each of them by repository
name and by a persistent identifier or direct link, together with the
author-generated code and artifacts required to reproduce the reported results.
The statement reads:
...
\begin{quotation}
\noindent
The data underlying the results presented in this study are publicly available.
Dataset~A, \emph{EEG data for Mental Attention State Detection}, is available
from Kaggle:
\url{https://www.kaggle.com/datasets/inancigdem/eeg-data-for-mental-attention-state-detection}.
Dataset~B, \emph{SAM~40}, is available from Figshare under the DOI
\url{https://doi.org/10.6084/m9.figshare.14562090.v1}. The author-generated
analysis code, the configuration of every reported run, and the per-run
artifacts underlying the reported figures and tables are available at
\url{https://github.com/sovetskiysn/eeg-validation-strategies} under the MIT
license and archived on Zenodo under the DOI
\url{https://doi.org/10.5281/zenodo.22255472}.
\end{quotation}

\vspace{0.2cm}

\noindent
No data underlying the reported findings are subject to access restrictions.
...
We thank the Editor for this clarification. Reviewer~\#3 recommended two
specific publications, both from 2025. We obtained and studied both papers and
assessed their relevance to the present work. We found them relevant to the
methodological context of our study, and we have accordingly cited and discussed
them in the Introduction of the revised manuscript, where they add pertinent
context on deep EEG decoding, on motor-imagery benchmarks, and on the challenges
of cross-subject and cross-task generalization that motivate our work.
